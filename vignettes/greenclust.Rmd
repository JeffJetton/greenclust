...
```
When building a model, you'll often need to reduce the number of levels present in a categorical (factor) variable. Typically this is done by combining certain levels together a smaller number of groups.

For example, a postal code predictor variable could easily have dozens of values or more, resulting in a model with a correspondingly large number of predictors. One option would be to lump the codes with a relatively small number of observations into a single "other" category. However it's possible that some of these low-frequency postal codes might contain information that would improve the final model. Grouping them in with other postal codes can dilute the predictive value of the variable.

The `greenclust` package provides an automatic way to combine categories/levels by iteratively clustering the categories two at a time. At each step, the pair is chosen whose combination results in the smallest reduction in the resulting contingency table's chi-squared statistic. This technique, sometimes called "Greenacre's Method", preserves as much information in the variable as possible for any given number of final groups. Levels with similar distributions of the target variable will tend to be grouped together.
...
For comparison, we'll create a variable of clustered neighborhoods using `greenclust`. It requires a contingency table as an argument:
...
Seven clusters looks pretty good, so we'll stick with that. While the `greencut` function will let you specify any cutpoint, by default (when no cutpoint is given) it will cut the tree at the same optimal point displayed by `greenplot`.
...
Since greenclust returns an hclust-compatible object, trees can be easily displayed using the base plot functions.
   
```{r plot_example, fig.width=7, fig.height=6}
plot(grc)
...
To get a sense of how `greenclust` clusters compare to simple thresholding, we'll build a few quick models with only `neighborhood` as a predictor.

```{r model_comp}
# Model using all levels, uncombined
fit.nocluster <- glm(highval ~ neighborhood, family="binomial", data=data)

summary(fit.nocluster)

# Model using count-based clustering
fit.countcluster <- glm(highval ~ countcluster, family="binomial", data=data)

summary(fit.countcluster)

# Model using greenclust clustering
fit.greencluster <- glm(highval ~ greencluster, family="binomial", data=data)

summary(fit.greencluster)
```
